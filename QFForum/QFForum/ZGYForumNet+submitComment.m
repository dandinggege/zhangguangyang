...
#import "ZGYForumNet+submitComment.h"

#import <UIKit/UIKit.h>

#import "ZGYUserManager.h"

@implementation ZGYForumNet (submitComment)
...
+(void)submitCommentWithTitleId:(NSString *)titleId
    ...
    //创建一个请求:
    NSString *URLStr=[NSString stringWithFormat:@"%@%@",ZGY_DOMAIN_NAME,ZGY_COMMENT_SUBMIT];
    ...
    NSString * md5Str=[ZGYFunction getMD5StrFromString:[NSString stringWithFormat:@"commentSubmitQF%@",timeStamp]];
    //用户
    ...
    //描述
    descryption=[descryption stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    //参数字符串
    NSMutableString * premStr=[[NSMutableString alloc]init];
    [premStr appendFormat:@"titleId=%@",titleId];
    [premStr appendFormat:@"&descryption=%@",descryption];
